...
function Hoodies() {
  ...
  const uniqueColors = Array.from(
    new Set(data.map((product: HoodiesType) => product.color))
  ).sort();
  const uniqueSizes = Array.from(
    new Set(data.map((product: HoodiesType) => product.size))
  ).sort();
  const uniqueBrands = Array.from(
    new Set(data.map((product: HoodiesType) => product.company))
  ).sort();
